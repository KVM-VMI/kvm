			   ...
			   struct x86_exception *exception)
{
	...
	t_gpa  = vcpu->arch.mmu->gva_to_gpa(vcpu, gpa, access, false,
					    exception);
			      ...
			      struct x86_exception *exception)
{
	...
	return vcpu->arch.walk_mmu->gva_to_gpa(vcpu, gva, access, false,
					       exception);
}
...
gpa_t kvm_mmu_gva_to_gpa_ignore_permissions(struct kvm_vcpu *vcpu, gva_t gva,
					   struct x86_exception *exception)
{
	return vcpu->arch.walk_mmu->gva_to_gpa(vcpu, gva, 0, true, exception);
}
				...
				struct x86_exception *exception)
{
	...
	gpa_t gpa = vcpu->arch.walk_mmu->gva_to_gpa(
		vcpu, addr, access | PFERR_FETCH_MASK, false, exception);
				...
				bool write)
{
	...
	*gpa = vcpu->arch.walk_mmu->gva_to_gpa(vcpu, gva, access, false,
					       exception);
...
{
	unsigned long ept_views_to_keep = 0;
