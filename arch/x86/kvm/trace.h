...
TRACE_EVENT(kvm_exit,
	TP_PROTO(unsigned int exit_reason, struct kvm_vcpu *vcpu, u32 view, u32 isa),
	TP_ARGS(exit_reason, vcpu, view, isa),
	...
	TP_printk("vcpu %u reason %s rip 0x%lx view %u info %llx %llx",
		  __entry->vcpu_id,
		 (__entry->isa == KVM_ISA_VMX) ?
		 __print_symbolic(__entry->exit_reason, VMX_EXIT_REASONS) :
		 __print_symbolic(__entry->exit_reason, SVM_EXIT_REASONS),
		 __entry->guest_rip, __entry->view, __entry->info1, __entry->info2)
);
