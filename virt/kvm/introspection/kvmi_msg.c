...
static const char *const msg_IDs[] = {
	...
	[KVMI_VM_GET_NEXT_AVAILABLE_GFN] = "KVMI_VM_GET_NEXT_AVAILABLE_GFN",
...
static int handle_vm_get_next_available_gfn(struct kvm_introspection *kvmi,
					    const struct kvmi_msg_hdr *msg,
					    const void *req)
{
	struct kvmi_vm_get_next_available_gfn_reply rpl;

	memset(&rpl, 0, sizeof(rpl));
	rpl.gfn = kvm_get_next_available_gfn(kvmi->kvm);
...
static int(*const msg_vm[])(struct kvm_introspection *,
	...
	[KVMI_VM_GET_NEXT_AVAILABLE_GFN] = handle_vm_get_next_available_gfn,
