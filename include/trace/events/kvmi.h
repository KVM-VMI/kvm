...
TRACE_EVENT(
	kvmi_event_reply,
	TP_PROTO(__u32 id, __u32 seq, __u16 action),
	TP_ARGS(id, seq, action),
	TP_STRUCT__entry(
		__field(__u32, id)
		__field(__u32, seq)
		__field(__u16, action)
	),
	TP_fast_assign(
		__entry->id = id;
		__entry->seq = seq;
	...
	TP_printk("%s seq %d %s",
		trace_print_symbols_seq(p, __entry->id, kvmi_event_symbol),
		__entry->seq,
		trace_print_symbols_seq(p, __entry->action, kvmi_action_symbol))
);
...
TRACE_EVENT(
	kvmi_set_gfn_access,
	TP_PROTO(__u64 gfn, __u8 access, __u16 slot),
	TP_ARGS(gfn, access, slot),
	TP_STRUCT__entry(
		__field(__u64, gfn)
		__field(__u8, access)
		__field(__u16, slot)
	...
	TP_printk("gfn %llx %s slot %d",
		  __entry->gfn, KVMI_ACCESS_PRINTK(), __entry->slot)
);
...
TRACE_EVENT(
	...
	TP_printk("vcpu %x cr %x old_value %llx new_value %llx",
		  __entry->vcpu,
		  __entry->cr,
		  __entry->old_value,
		  __entry->new_value
...
TRACE_EVENT(
	...
	TP_printk("vcpu %x %s new_value %llx",
		  __entry->vcpu,
		  trace_print_symbols_seq(p, __entry->action,
					  kvmi_action_symbol),
		  __entry->new_value
...
TRACE_EVENT(
	...
	TP_printk("vcpu %x msr %x old_value %llx new_value %llx",
		  __entry->vcpu,
		  __entry->msr,
		  __entry->old_value,
		  __entry->new_value
...
TRACE_EVENT(
	...
	TP_printk("vcpu %x gpa %llx rip %llx",
		  __entry->vcpu,
		  __entry->gpa,
		  __entry->old_rip
	)
...
TRACE_EVENT(
	...
	TP_printk("vcpu %x gpa %llx %s gva %llx rip %llx",
		  __entry->vcpu,
		  __entry->gpa,
		  KVMI_ACCESS_PRINTK(),
		  __entry->gva,
...
TRACE_EVENT(
	kvmi_event_pf_recv,
	TP_PROTO(__u16 vcpu, __u32 action, __u64 next_rip, __u32 ctx_size,
		 bool singlestep, bool ret),
	TP_ARGS(vcpu, action, next_rip, ctx_size, singlestep, ret),
	TP_STRUCT__entry(
		__field(__u16, vcpu)
		__field(__u32, action)
	...
	TP_printk("vcpu %x %s rip %llx ctx %u%s",
		  __entry->vcpu,
		  trace_print_symbols_seq(p, __entry->action,
					  kvmi_action_symbol),
		  __entry->next_rip, __entry->ctx_size,
		  (__entry->singlestep ? (__entry->ret ? " SS->failed" :
							  " SS->running")
					: "")
	)
);
...
TRACE_EVENT(
	...
	TP_printk("vcpu %x vector %x/%x err %x/%x address %llx",
		  __entry->vcpu,
...
TRACE_EVENT(
	...
	TP_PROTO(__u16 vcpu, __u32 exit_info, __u64 exit_qualification,
		 __u8 descriptor, __u8 write),
	TP_ARGS(vcpu, exit_info, exit_qualification, descriptor, write),
	TP_STRUCT__entry(
		__field(__u16, vcpu)
	...
	TP_printk("vcpu %x %s %s exit_info %x exit_qualification %llx",
		  __entry->vcpu,
...
TRACE_EVENT(
	...
	TP_printk("vcpu %x gpa %llx %s insn %s level %x",
		  __entry->vcpu_id,
		  __entry->gpa,
		  KVMI_ACCESS_PRINTK(),
...
TRACE_EVENT(
	...
	TP_printk("vcpu %x %s address %llx error %x",
		  __entry->vcpu_id,
